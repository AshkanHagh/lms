...
export type InsectCourseDetails = Omit<
  TSelectCourse,
  "id" | "updatedAt" | "createdAt"
>;

export type InsectCourseDetailsBody<T> = T extends "insert"
  ? Pick<TSelectCourse, "title" | "teacherId">
  : Omit<TSelectCourse, "id" | "createdAt" | "updatedAt" | "prerequisite"> & {
...
export type courseBenefitAndDetails = {
  benefits: TSelectCourseBenefit[];
  course: TSelectCourse;
};

export type TStudentResultClient = Omit<
  TSelectStudent,
  "updatedAt" | "createdAt" | "customerId"
>;
...
export type TSendToken = {
  accessToken: string;
  sanitizedStudent: TStudentResultClient;
};

export type TokenResponse<T> = T extends "refresh"
  ? { accessToken: string }
  : TSendToken;
export type SelectCondition<T> = T extends "emailOnly"
  ? Pick<TModifiedStudent, "email">
  : TSelectStudent;
...
export type insertChapterBody = {
  ...
  chapterDetails: ModifiedChapterDetail;
};

export type ChapterAndVideoDetails = {
  videoDetail: TSelectVideoDetails[];
  chapterDetails: TSelectChapter;
};
...
export type InsertVideoDetails = Pick<
  TSelectVideoDetails,
...
export type ModifiedPurchase = Omit<
  TSelectPurchases,
  "brand" | "card" | "expMonth" | "expYear" | "paymentId"
>;
...
export type CheckPurchaseValue<T> = T extends "modified"
  ? ModifiedPurchase
  : TSelectPurchases;

export type CourseRelations =
  | (TSelectCourse & {
      benefits: TSelectCourseBenefit[] | null;
      chapters: (TSelectChapter & { videos: TSelectVideoDetails[] })[] | null;
      tags: TSelectTags[];
      teacher: Teacher | null;
      purchases: { studentId: string | null }[] | null;
    })
  | undefined;

export type ModifiedCourseRelations =
  | (TSelectCourse & {
      benefits: TSelectCourseBenefit[] | null;
      chapters: (TSelectChapter & { videos: TSelectVideoDetails[] })[] | null;
      tags: TSelectTags[];
...
export type FilteredChapters =
  | (TSelectChapter & { videos: TSelectVideoDetails[] })[]
  | undefined;
...
export type PurchaseDetailRes = Omit<TSelectPurchases, "courseId"> & {
  course: Pick<TSelectCourse, "title" | "price" | "id">;
};
...
export type CourseStateResult = {
  remainingVideos: TSelectVideoDetails[];
  progressPercentage: number;
};

export type MostUsedTagsMap = { tag: TSelectTags; count: number };

export type VectorSeed = Pick<
  TSelectCourse,
  "id" | "description" | "title" | "visibility" | "image" | "price"
>;
export type VectorResult = { score: number; course: VectorSeed };

export type TransactionResult = {
  modifiedPurchase: PurchaseDetailRes[];
  subscriptionDetail: TSelectSubscription;
};
...
export type CoursesWithPurchaseDetail = TSelectCourse & {
  purchases: Pick<TSelectPurchases, "studentId">[];
};
...
export type CoursesProgress = Omit<ModifiedRelationsCourse, "chapters"> & {
  progress: number;
};
...
export type CommentAuthorDetail = Pick<
  TSelectStudent,
  "id" | "image" | "name" | "role"
>;
export type ModifiedCommentResult = Omit<TSelectComment, "authorId"> & {
  author: CommentAuthorDetail;
};
...
export type ModifiedSendReplay = Omit<TSelectReplay, "authorId"> & {
  author: CommentAuthorDetail;
};
