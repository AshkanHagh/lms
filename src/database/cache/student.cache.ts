...
export const findStudentStates = async <T>(
  currentStudentId: string,
  coursesId: string[],
): Promise<T[]> => {
  const pipeline: ChainableCommander = redis.pipeline();
  coursesId.forEach((id) =>
    pipeline.hgetall(`student_state:${currentStudentId}:course:${id}`),
  );
  return (await pipeline.exec())!
    .map((course) => course[1])
    .filter((courseData) => Object.keys(courseData!).length > 0) as T[];
};

export const findTeacherCoursesCache = async (
...
export const findCommentAuthor = async (
  studentIds: string[],
): Promise<CommentAuthorDetail[]> => {
  const pipeline: ChainableCommander = redis.pipeline();
  studentIds.forEach((id) => pipeline.hgetall(`student:${id}`));

  const studentsDetail = (await pipeline.exec())!.map(
    (student) => student[1],
  ) as TSelectStudent[];

  return studentsDetail.map((student) => ({
    id: student.id,
    name: student.name,
    role: student.role,
    image: student.image,
