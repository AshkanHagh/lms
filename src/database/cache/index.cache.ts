...
export const insertHashCache = async <T>(
  hashKey: string,
  keyValue: TInsertCache<T>,
): Promise<void> => {
...
export const getHashCache = async <T>(
  hashKey: string,
  hashIndex: string,
): Promise<T> => {
  await redis.expire(hashKey, 604800);
...
export const insertHashListCache = async <T>(
  hashKey: string,
  hashIndex: string,
  hashValue: T,
): Promise<void> => {
...
export const removeFromHashListCache = async (
  hashKey: string,
  hashIndex: string,
): Promise<void> => {
...
export const insertSetListCache = async (
  setKey: string,
  setValue: string,
): Promise<void> => {
