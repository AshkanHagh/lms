...
export const findAllCourseChapter = async (courseId : string, chapterId : string) => {
    let cursor : string = '0';
    ...
    do {
        const [newCursor, keys] : [string, string[]] = await redis.scan(cursor, 'MATCH', `course:${courseId}:*`, 'COUNT', 100);
