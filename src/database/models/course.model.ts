...
export const courseTable = pgTable(
  "courses",
  {
    ...
    teacherId: uuid("teacher_id").references(() => studentTable.id, {
      onDelete: "cascade",
    }),
  ...
  (table) => ({
    titleIndex: index("course_title_index").on(table.title),
    descriptionIndex: index("course_index").on(table.description),
    teacherIndex: index("course_teacherId_index").on(table.teacherId),
  }),
);

export const courseBenefitTable = pgTable(
  "benefit",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const chapterVideosTable = pgTable(
  "videos",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const courseTagsTable = pgTable(
  "tags",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    courseId: uuid("course_id").references(() => courseTable.id, {
...
export const completeState = pgTable(
  "complete_state",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    studentIdIndex: index("complete_studentId_index").on(table.studentId),
    courseIdIndex: index("complete_course_id").on(table.courseId),
  }),
);
...
export const courseBenefitTableRelations = relations(
  courseBenefitTable,
  ({ one }) => ({
    course: one(courseTable, {
...
export const courseChaptersTableRelations = relations(
  courseChaptersTable,
  ({ one, many }) => ({
    course: one(courseTable, {
...
export const chapterVideosTableRelations = relations(
  chapterVideosTable,
  ({ one }) => ({
...
export const courseTagsTableRelations = relations(
  courseTagsTable,
  ({ one }) => ({
    course: one(courseTable, {
