...
export const courseRatingTable = pgTable(
  "ratings",
  {
...
export const commentTable = pgTable(
  "comments",
  {
    ...
    authorId: uuid("author_id").references(() => studentTable.id, {
      onDelete: "cascade",
    }),
  ...
  (table) => ({
    authorIdIndex: index("authorId_index_comment").on(table.authorId),
  }),
);
...
export const courseCommentsTable = pgTable(
  "course_comments",
  {
    courseId: uuid("course_id").references(() => courseTable.id, {
      onDelete: "cascade",
  ...
  (table) => ({
    pk: primaryKey({ columns: [table.courseId, table.commentId] }),
  }),
);

export const courseRatingTableRelations = relations(
  courseRatingTable,
  ({ one }) => ({
...
export const courseCommentsTableRelations = relations(
  courseCommentsTable,
  ({ one }) => ({
    course: one(courseTable, {
