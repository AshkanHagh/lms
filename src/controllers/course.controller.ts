...
import {
  courseBenefitService,
  courseChapterDetailsService,
  courseService,
  coursesService,
  courseStateDetailService,
  courseVideosDetailService,
  createCourseChapterService,
  createCourseService,
  editCourseDetailsService,
  filterCourseByTagsService,
  markAsCompletedService,
  mostUsedTagsService,
  updateChapterVideoDetailService,
  updateCourseChapterService,
  vectorSearchService,
} from "../services/course.service";

export const createCourse = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { title } = req.body as InsectCourseDetailsBody<
      CourseGeneric<"insert">
    >;
...
export const courseBenefit = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { benefits } = req.body as Pick<courseBenefitAndDetails, "benefits">;
    const courseDetail: TSelectCourse = req.course;

    const benefitWithCourseId: Omit<TSelectCourseBenefit, "id">[] =
      benefits.map<Omit<TSelectCourseBenefit, "id">>((benefit) => ({
        title: benefit.title,
...
export const createCourseChapter = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { courseId } = req.params as CourseParams;
...
export const updateCourseChapter = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { visibility, description, title } =
      req.body as Partial<ModifiedChapterDetail>;
    const { courseId, chapterId } = req.params as CourseAndChapterId;

    const chapterDetail: TSelectChapter = await updateCourseChapterService(
      chapterId,
      courseId,
      {
        visibility,
...
export const updateChapterVideoDetail = CatchAsyncError(
  async (req: Request, res: Response) => {
...
export const courseChapterDetails = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { courseId, chapterId } = req.params as CourseAndChapterId;
    const { plan, id }: TSelectStudent = req.student!;

    const chapterDetails: ChapterDetails = await courseChapterDetailsService(
      courseId,
      chapterId,
      { plan, id },
    );
...
export const courseVideosDetail = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { videoId, chapterId } = req.params as ChapterAndVideoId;
    const currentStudentId: string = req.student!.id;

    const videoDetail: TSelectVideoDetails = await courseVideosDetailService(
      videoId,
      chapterId,
      currentStudentId,
    );
...
export const markAsCompleted = CatchAsyncError(
  async (req: Request, res: Response) => {
    ...
    const videoCompleteStateDetail: SelectVideoCompletion =
      await markAsCompletedService(videoId, courseId, currentStudentId, state);
    res.status(200).json({ success: true, videoCompleteStateDetail });
  },
);

export const courseStateDetail = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { courseId } = req.params as CourseParams;
...
export const mostUsedTags = CatchAsyncError(
  async (req: Request, res: Response) => {
...
export const filterCourseByTags = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { tags } = req.body as { tags: string[] };
    const similarCourse: TSelectCourse[] =
      await filterCourseByTagsService(tags);
