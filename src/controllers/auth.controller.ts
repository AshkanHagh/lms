...
export const verifyAccount = <T extends "login" | "register">(condition: T) => {
  return CatchAsyncError(async (req: Request, res: Response) => {
    ...
    const verifyResult: TSelectStudent = await verifyAccountService(
      activationToken,
      activationCode,
      condition,
    );

    const { accessToken, sanitizedStudent } = await sendToken(
      verifyResult,
      res,
      "login",
    );
    res
      .status(200)
      .json({ success: true, student: sanitizedStudent, accessToken });
  });
};
...
export const socialAuth = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { name, email, image } = req.body as Pick<
      TModifiedStudent,
      "name" | "email" | "image"
    >;
    const studentDetails = await socialAuthService(
      name!.toLowerCase(),
      email.toLowerCase(),
      image!,
    );
