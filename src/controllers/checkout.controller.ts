...
import {
  ...
  webhookListeningService,
} from "../services/checkout.service";
import type {
  CompletePaymentQueries,
  TSelectPurchases,
  TSelectStudent,
...
export const checkout = CatchAsyncError(async (req: Request, res: Response) => {
  ...
  const paymentUrl: string | null = await Promise.race([
    checkoutService(currentStudent, courseId),
    timeout,
  ]);
...
export const verifyPayment = CatchAsyncError(
  async (req: Request, res: Response) => {
    const { session_id, course_id, student_id } =
      req.query as CompletePaymentQueries;
    const verifiedPurchase: TSelectPurchases = await verifyPaymentService(
      session_id,
      course_id,
      student_id,
    );
...
export const cancelPayment = CatchAsyncError(
  ...
  async (req: Request, res: Response) => {
    res
      .status(200)
      .json({ success: true, message: "Purchase was canceled successfully" });
  },
);

export const webhookEventListener = CatchAsyncError(
  async (req: Request, res: Response) => {
...
export const subscriptionCheckout = CatchAsyncError(
  async (req: Request, res: Response) => {
    ...
    const checkoutUrl: string | null = await Promise.race([
      subscriptionCheckoutService(plan, currentStudent),
      timeout,
    ]);
...
export const subscriptionPortal = CatchAsyncError(
  async (req: Request, res: Response) => {
