...
class CustomerIdNotFoundError extends ErrorHandler {
  constructor(
    message: string = "CustomerId not found. you must subscribe to a plan to access this recourse",
  ) {
    super(message, 404);
  }
}
...
class SubscriptionNotFoundError extends ErrorHandler {
  constructor(
    message: string = "Subscription not found. you must subscribe to a plan to access this recourse",
  ) {
    super(message, 404);
  }
}
...
class PaymentFailedError extends ErrorHandler {
  constructor(
    ...
    super(
      `Payment failed for invoice ${invoiceId}: ${failure_message || "Unknown reason"}. Please check the payment method and try again.`,
      400,
    );
  }
}
