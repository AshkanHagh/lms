...
import {
  ...
  updateCommentDetail,
  updateReplayDetail,
  type ModifiedRepliesWithAuthor,
} from "../database/queries/comment.query";
...
export const rateCourseService = async (
  ...
  try {
    const studentPurchaseCache: number = await getSetListCache(
      `course_purchases:${courseId}`,
      currentStudentId,
    );
    const studentHasPurchased: number = studentPurchaseCache
      ? studentPurchaseCache
...
export const courseRateDetailService = async (
  courseId: string,
): Promise<number> => {
  try {
    const rateCache: Record<string, string> = await getAllHashCache(
      `course_rate:${courseId}`,
    );
    const courseRateDetail: TSelectRate[] = rateCache
      ? Object.values(rateCache)
          .map((rate) => JSON.parse(rate) as TSelectRate[])
          .flat()
      : await findRatesDetail(courseId);

    const rates: TSelectRate[] = courseRateDetail.filter(
      (rate) => rate.rate !== null,
    );
    const totalRates: number = rates.reduce(
...
export const sendCommentService = async (
  student: CommentAuthorDetail,
  courseId: string,
  text: string,
): Promise<ModifiedCommentResult> => {
  try {
    const commentDetail: TSelectComment = await handelInsertComment(
      student.id,
      courseId,
      text,
    );
...
export const updateCommentService = async (
  ...
  commentId: string,
  text: string,
): Promise<TSelectComment> => {
  try {
    const checkCommentAuthor = JSON.parse(
      await getHashCache<string>(`course_comments:${courseId}`, commentId),
    ) as TSelectComment;

    if (!checkCommentAuthor || checkCommentAuthor.authorId !== studentId) {
...
export const deleteCommentService = async (
  ...
  try {
    ...
    await Promise.all([
      ...
      removeFromHashListCache(`course_comments:${courseId}`, commentId),
    ]);
...
export const courseCommentsService = async (
  courseId: string,
  limit: number,
  startIndex: number,
): Promise<ModifiedCommentResult[]> => {
  try {
    const commentsDetailCache: Record<string, string> = await getAllHashCache(
      `course_comments:${courseId}`,
    );
    ...
    const commentAuthors: CommentAuthorDetail[] = await findCommentAuthor(
    ...
    return commentDetail.map((comment) => {
      ...
      return {
        ...rest,
        author: commentAuthors.find((author) => author.id === authorId)!,
      };
    });
  } catch (err: unknown) {
    const error = err as TErrorHandler;
...
export const sendReplayService = async (
  student: CommentAuthorDetail,
  courseId: string,
  commentId: string,
  text: string,
  ...
  try {
    ...
    const replayDetail: TSelectReplay = await insertReplay(
      commentId,
      student.id,
      text,
    );
...
export const updateReplayService = async (
  student: CommentAuthorDetail,
  commentId: string,
  replayId: string,
  text: string,
...
export const removeReplayService = async (
  studentId: string,
  commentId: string,
  replayId: string,
): Promise<string> => {
  try {
    ...
    if (!replayCache || replayCache.authorId !== studentId) {
      throw new BadRequestError();
    }
...
export const repliesDetailService = async (
  commentId: string,
  limit: number,
  startIndex: number,
) => {
  try {
    ...
    const repliesDetail: ModifiedRepliesWithAuthor[] = sortedAndParsedReplies
      ...
      : await findReplies(commentId, limit, startIndex);
